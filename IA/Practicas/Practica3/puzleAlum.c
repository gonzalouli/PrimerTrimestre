...
int objetivo=1;
    ...
    if( objetivo=iguales( estado, estadoObjetivo()))
...
int piezasmalcolocadas(tEstado *estado){
  ...
  for(int i=0; i<N ; i++){
      for(int j=0 ; j<N ; j++){
        ...
        if(estado->celdas[i][j]==(i+j+1)%(N*N)){
          if(h!=0)
            h--;
...
int distanciamanhattan(tEstado *estado){
  ...
  for( int i=1 ; i <= 9 ;i++){
    filas = abs(estado->fila[i] - objetivo->fila[i]);
    columnas = abs(estado->col[i] - objetivo->col[i]);
    distanciamanhattan= distanciamanhattan + filas + columnas;
  }
